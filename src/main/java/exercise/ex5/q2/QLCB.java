package exercise.ex5.q2;

import exercise.ex5.q1.*;
...
public class QLCB {
    ...
    public void showMenu() {
        while (true) {
            ...
            if (menu == 1) themCanBo();
            else if (menu == 2) timKiemTheoHoTen();
            else if (menu == 3) hienThiDSCB();
            else if (menu == 4) xoaTheoTen();
    ...
    private void xoaTheoTen() {
        ...
        Predicate<CanBo> predicate = new Predicate<CanBo>() {
            ...
            public boolean test(CanBo canBo) {
                // Nguyen Van Khoa
                int lastSpaceIndex = canBo.getHoTen().lastIndexOf(' ');
                String tenCanBo = canBo.getHoTen().substring(lastSpaceIndex + 1);
    ...
    private GioiTinh nhapVaoGioiTinh() {
        ...
        if (menu.equals("1")) return GioiTinh.NAM;
        else if (menu.equals("2")) return GioiTinh.NU;
